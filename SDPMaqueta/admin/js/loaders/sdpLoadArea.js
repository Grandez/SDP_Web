/*
  ...
  1 - Stats
 */
function refreshArea($scope, $state, datos) {

    $scope.node = getSDP($scope).getArea();
    $scope.node.alertas = datos[0];
    $scope.node.stats   = datos[1];
/*
...
function refreshAppl($scope, $state,  datos) {
    $scope.node = getSDP($scope).getAppl();
    $scope.node.alertas = datos[0];
    $scope.node.stats   = datos[1];
}
